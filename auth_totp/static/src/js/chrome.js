 ...
 *---------------------------------------------------------*/
/*---------------------------------------------------------
 * Modifications for TOTP based login are marked with
 *           'customized for TOTP login'
 *---------------------------------------------------------*/
...
instance.web.Login =  instance.web.Widget.extend({
    ...
    on_submit: function(ev) {
		...
		// customized for TOTP login - begin
		var totp_pin = this.$("form input[name=totp_pin]").val();
        this.do_login(db, login, password, totp_pin);
		// customized for TOTP login - end
    },
    /**
     ...
     * @param {String} totp_pin TOTP PIN code (based on user secret)
     */
	// customized for TOTP login - begin
	do_login: function (db, login, password, totp_pin) {	
	// customized for TOTP login - end
        var self = this;
        self.hide_error();
        self.$(".oe_login_pane").fadeOut("slow");
		// customized for TOTP login - begin
        return this.session.session_authenticate(db, login, password, totp_pin).then(function() {
		// customized for TOTP login - end
